...
public:
	...
	CChannelParam* m_pDecodeChParam[MAX_BOARD_CHANNEL];
	...
	sdvr_err_e StartEncodeChannel(int iCh);	// Set parameter and Start Encode to channel.
	sdvr_err_e StopEncodeChannel(int iCh);	// Stop Encode to channel.

	sdvr_err_e CreateDecodeChannel(int iCh, int iWidth = 720, int iHeight = 480);	// Set parameter and Start Decode to channel.
	sdvr_err_e DeleteDecodeChannel(int iCh);	// Stop Decode to channel.

	sdvr_err_e StartDecodeChannel(int iCh);	// Set parameter and Start Decode to channel.
	sdvr_err_e StopDecodeChannel(int iCh);	// Stop Decode to channel.

	BOOL     SetVideoEncodeParam(int iCh);	

	int		m_iTotalHWEncodeChannel[MAX_CHIP_NUM];
	int		m_iTotalHWDecodeChannel[MAX_CHIP_NUM];
