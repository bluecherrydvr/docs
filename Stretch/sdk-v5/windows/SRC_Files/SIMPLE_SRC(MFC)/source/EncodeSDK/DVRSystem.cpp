...
sdvr_err_e CDVRSystem::CreateEncodeChannel(int iCh)
{
	...
	if (!m_pEncodeChParam[iCh])
	{
		...
		m_pEncodeChParam[iCh]->m_pMainDlg = m_pMainDlg;
...
sdvr_err_e CDVRSystem::StartEncodeChannel(int iCh , int subEnc)
{
	sdvr_err_e sdvr_rc = SDVR_ERR_NONE;	

	if (m_pEncodeChParam[iCh])
	{
		m_pEncodeChParam[iCh]->EnableRecChannel(true , subEnc);
...
sdvr_err_e CDVRSystem::StartEnableChannel(int iCh)
{
	sdvr_err_e sdvr_rc = SDVR_ERR_NONE;	
...
sdvr_err_e CDVRSystem::StopPreviewChannel(int iCh)
{
	sdvr_err_e sdvr_rc = SDVR_ERR_NONE;	
	...
	if (m_pEncodeChParam[iCh])
	{
		...
		while(m_pEncodeChParam[iCh]->m_bIsClosingChannel)
		{
			...
			iWhileLoopCnt++;
			if(iWhileLoopCnt > 50) //Maybe callback don't work
			{
				m_pEncodeChParam[iCh]->m_bIsClosingChannel = false;
				break;
			}
		}

		m_pEncodeChParam[iCh]->EnableChannel(false);
...
sdvr_err_e CDVRSystem::StopEncodeChannel(int iCh , int subEnc)
{
	sdvr_err_e sdvr_rc = SDVR_ERR_NONE;	
	...
	if (m_pEncodeChParam[iCh])
	{
		...
		m_pEncodeChParam[iCh]->EnableRecChannel(false , subEnc);
		m_iTotalHWEncodeChannel[iCh/4] --;
...
sdvr_err_e CDVRSystem::ConnectChannel(int iCh)
{
	...
	if (!m_bEnableChannel[iCh])
	{
		...
		m_pChannelParam[iCh]->SetChannelParamAndCreate(CHANNEL_VDO_DATA_TYPE_RAW);

		if (sdvr_rc == SDVR_ERR_NONE)
		{		
			m_bEnableChannel[iCh] = true;
			sprintf_s(szDebug, sizeof(szDebug), "Channel(%d) connect OK.....", iCh);
			m_pMainDlg->SetWindowText(szDebug);
			OutputDebugString(szDebug);
		}
		else
		{
			sprintf_s(szDebug, sizeof(szDebug), "Channel(%d) connect fail(%d).....", iCh, sdvr_rc);
			m_pMainDlg->SetWindowText(szDebug);
			OutputDebugString(szDebug);
		}
	}
...
sdvr_err_e CDVRSystem::ResetFrameRate(int iCh , int iFrame)
{
	sdvr_err_e sdvr_rc = SDVR_ERR_NONE;	
	char szDebug[200];

	sdvr_rc = sdvr_stream_raw_video(m_pChannelParam[iCh]->m_chan_handle, m_video_res_decimation[iCh], iFrame, true);
	if (sdvr_rc == SDVR_ERR_NONE)
	{
		sprintf_s(szDebug, sizeof(szDebug), "Board(%d) channel(%d) ResetFrameRates(%d) = SDVR_ERR_NONE\n", m_iBoardIndex, iCh, m_iFrameRate[iCh]);
		OutputDebugString(szDebug);		
	}
	else
	{
		sprintf_s(szDebug, sizeof(szDebug), "Board(%d) channel(%d) ResetFrameRate(%d) fail(%d).....\n", m_iBoardIndex, iCh, m_iFrameRate[iCh], sdvr_rc);
		OutputDebugString(szDebug);		
	}
...
sdvr_err_e CDVRSystem::SetOSD(int iCh)
{
	...
	for (i=0; i<SDVR_MAX_OSD; i++)
	{
		...
		sdvr_rc = sdvr_osd_text_show(m_pEncodeChParam[iCh]->m_chan_handle, i, m_bEnableOSD[iCh][i] && bCanSupportNewOSD);
	}
...
bool CDVRSystem::CheckVideoFormatSize()
{
	...
	{		
		if (m_video_res_decimation[i] == SDVR_VIDEO_RES_DECIMATION_EQUAL)
		{
			...
			m_pMainDlg->m_iVdoWidth_Enc[i] = 720;
			m_pMainDlg->m_iVdoHeight_Enc[i] = 480;
		}
		else if (m_video_res_decimation[i] == SDVR_VIDEO_RES_DECIMATION_CIF)
		{
			if(m_bIsPAL)
			{
				m_pMainDlg->m_iVdoWidth_Raw[i] = 352;
				m_pMainDlg->m_iVdoHeight_Raw[i] = 288;
				m_pMainDlg->m_iVdoWidth_Enc[i] = 720;
				m_pMainDlg->m_iVdoHeight_Enc[i] = 576;
			}
			else
			{
				m_pMainDlg->m_iVdoWidth_Raw[i] = 352;
				m_pMainDlg->m_iVdoHeight_Raw[i] = 240;
				m_pMainDlg->m_iVdoWidth_Enc[i] = 720;
				m_pMainDlg->m_iVdoHeight_Enc[i] = 480;
			}
		}
