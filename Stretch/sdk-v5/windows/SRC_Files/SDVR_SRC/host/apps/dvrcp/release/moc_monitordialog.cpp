/****************************************************************************
** Meta object code from reading C++ file 'monitordialog.h'
**
** Created: Wed Dec 24 15:05:18 2008
**      by: The Qt Meta Object Compiler version 59 (Qt 4.2.2)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../monitordialog.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'monitordialog.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 59
#error "This file was generated using the moc from 4.2.2. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

static const uint qt_meta_data_MonitorDialog[] = {

 // content:
       1,       // revision
       0,       // classname
       0,    0, // classinfo
       2,   10, // methods
       0,    0, // properties
       0,    0, // enums/sets

 // slots: signature, parameters, type, tag, flags
      15,   14,   14,   14, 0x08,
      30,   24,   14,   14, 0x08,

       0        // eod
};

static const char qt_meta_stringdata_MonitorDialog[] = {
    "MonitorDialog\0\0accept()\0index\0"
    "onDecimationComboActivated(int)\0"
};

const QMetaObject MonitorDialog::staticMetaObject = {
    { &QDialog::staticMetaObject, qt_meta_stringdata_MonitorDialog,
      qt_meta_data_MonitorDialog, 0 }
};

const QMetaObject *MonitorDialog::metaObject() const
{
    return &staticMetaObject;
}

void *MonitorDialog::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_MonitorDialog))
	return static_cast<void*>(const_cast<MonitorDialog*>(this));
    return QDialog::qt_metacast(_clname);
}

int MonitorDialog::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QDialog::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        switch (_id) {
        case 0: accept(); break;
        case 1: onDecimationComboActivated((*reinterpret_cast< int(*)>(_a[1]))); break;
        }
        _id -= 2;
    }
    return _id;
}
