...
/************************************************************************
    ...
    sdvr_player_close_file() is called for a port playing a file, and sdvr_player_close_stream() 
    ...
    int main()
    {
        ...
        sdvr_player_open_file(FILE_PORT, "myfile.raw");
...
/**********************************************************************************
  VISIBLE: SDVR player error code. 
    ...
    @SDVR_PLAYER_INITIALIZED@ - Player is already initialized.
    ...
    @SDVR_PLAYER_UNSUPPORTED_FILE_FORMAT@ - Input file format is not supported.
...
typedef enum 
{
    SDVR_PLAYER_NOERROR = 3000,		        // no error, operation succeeded.
    SDVR_PLAYER_NULL_POINTER,               // null pointer.
    SDVR_PLAYER_ALLOC_MEMORY_ERROR,         // memory allocation failed.
    SDVR_PLAYER_NOT_IMPLEMENTED,            // feature not implemented.
    SDVR_PLAYER_INVALID_PARAM,              // invalid input parameter.
...
*******************************************************************************/
EXTERN void sdvr_player_get_version(sx_uint8 *major, sx_uint8 *minor, sx_uint8 *revision, sx_uint8 *build);
