...
{
	char*       str1 = "scode demo - loop test for solo6110";
	GdkFont*    font = NULL;
	GdkColor    color;
	char        title_str[160];
	char        tmpstr[] = " ";
...
void* videorecord(void* arg)
{
    ...
    unsigned char tmp;
    struct timeb timebuffer;    
    int enc_fd = open("/dev/solo6110_enc0", O_RDWR);
...
/*                    GetEncodePara(ChannelHandle[selectport],
        ...
        if((buf[7]>>1)&3)
	{
	    ...
	    read_size -= 256;       
            ...
            ret_val = write(record_fd[0], buf, 64);
            if(ret_val != 64)
            {
                printf("write(%d, %d)\n", ret_val, 64);
                return 0;
	...
	read_size -= 64;
	ret_val = write(record_fd[0], buf+64+256, read_size);
...
void* videodecode(void* arg)
{
	...
	else
	{
        ...
        VideoDecode(dec_buf, header_size + mpeg4_size);
        usleep(1000);
