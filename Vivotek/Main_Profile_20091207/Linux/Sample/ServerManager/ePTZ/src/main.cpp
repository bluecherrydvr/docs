...
int main()
{
	...
	// STEP 7: Set PTZ type
	// Param ePTZType eSrvCtrlPPTZ for physical PTZ; eSrvCtrlEPTZ for digital PTZ
	ServerManager_SetPTZType(hDevice, eSrvCtrlEPTZ);
	...
	// STEP 8: Move camera use ePTZ
	// Param dwCamera  In the multiple camera model, it indicates the order from the right with starting as 1. 
	//				   Otherwise it will be ignored
	//
	// Param dwStream  It indicates resolution of which stream will be get for computing the target position.
	//
	...
	// STEP 9: Send PTZ command to server use ePTZ
	// Param dwCamera    MAKELONG(x, y) uses in ePTZ
	//						: In the multiple camera model, the x indicate the the order 
	//						  from the right with starting as 1; the y indicate the resolution 
	//						  of which stream will be get for ePTZ command.
	//
	...
	// Notice : You have to connect the camera with ePTZ capability
	{
		scRet = ServerManager_SendPTZCommandBlock(hDevice, MAKELONG(1, dwStream), eSrvCtrlPTZMoveLeft, "", TRUE);
		if (scRet != S_OK)
		{
