...
#include "NonBlockingSendHTTPCommand.h"
...
SCODE __stdcall ProcNotify(TSVRCTRL_NOTIFY_CONT *ptContext)
{
	...
	else if (strcmp(ptCBContent->szNotice, "ServerManager_SendHttpCommand") == 0)
	{
		printf("ServerManager_SendHttpCommand success\n");	
	}
...
int main()
{
	...
	{
		// Set notify string
		strcpy(tCBContent.szNotice, "ServerManager_SendHttpCommand");
		scRet = ServerManager_SendHttpCommand(hDevice, "/cgi-bin/admin/setparam.cgi?system_hostname=Network Camera", TRUE, &tNotify, NULL);
		if (scRet != S_OK)
		{
