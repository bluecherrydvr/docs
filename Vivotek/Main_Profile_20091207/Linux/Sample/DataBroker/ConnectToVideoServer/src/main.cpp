...
{	
	...
	//Set camera index to video server
	printf("Please choose different camra (camera1 = 1 , camera2 = 2 , camera3 = 3 , camera4 = 4 , all camera = 5) : ");
	_tscanf("%hu", &(ptOpt->wCam));
	...
	ptOpt->dwFlags = eConOptProtocolAndMediaType | eConOptHttpPort | eConOptCam |
...
int main()
{
	...
	if (IS_FAIL(scRet))
	{
        ...
        // Press any key to stop receive packet
	while( !_kbhit() )
	{      
		// sleep 16 ms
		Sleep(16);
