...
{	
	...
	//Set device HTTPS port
	printf("Please input device HTTPS port (default is 443): ");
	_tscanf("%hu", &(ptOpt->wHttpPort));

	//Using HTTPS protocol, protocol type  must be HTTP, 
	ptOpt->dwProtocolType = eptHTTP;
...
int main()
{
	...
	// STEP 4: Set extra option for enable HTTPS protocol
	scRet = DataBroker_SetConnectionExtraOption(hConn, eOptSSLEnable, TRUE, NULL);
