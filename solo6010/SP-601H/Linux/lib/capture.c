...
#include "scode.h"
#include "../module/include/solo6010/enc.h"
#include "../module/include/solo6010/disp.h"
#include "../module/include/solo6010/dec.h"
...
int solo6010_display_set_norm(SOLO6010_DISPLAY *disp, int norm)
{
    return ioctl(disp->fd, IOCTL_DISP_SET_NORM, &norm);
}

int solo6010_display_motion_trace(SOLO6010_DISPLAY *disp, int on)
{
	if(on)
...
void solo6010_display_channel_mode(SOLO6010_DISPLAY *disp, unsigned int channel, unsigned int mode)
{
	disp->disp_ctrl[channel].mode = mode;
	ioctl(disp->fd, IOCTL_DISP_CHANNEL, &disp->disp_ctrl[channel]);
}

int solo6010_display_zoom(SOLO6010_DISPLAY *disp, int on, int x, int y)
{
...
int solo6010_display_mux_mode(SOLO6010_DISPLAY *disp, unsigned int mode)
{
...
SOLO6010_DISPLAY *solo6010_display_open(const char *path, unsigned int mode)
{
	...
	disp = malloc(sizeof(SOLO6010_DISPLAY));
	if(!disp)
		return NULL;

	bzero(disp, sizeof(SOLO6010_DISPLAY));
	...
	disp->pip_ctrl = NULL;
	solo6010_display_zoom(disp, 0, 0, 0);
	solo6010_display_mux_mode(disp, mode);
	return disp;
}
